...
# -------------------------------------------------------
df <- read.csv("data/youtube_title_description_comments.csv")
df
# 데이터 클렌징
...
ud_model <- udpipe_load_model("korean-gsd-ud-2.5-191206.udpipe")

# 텍스트에 어근 통일화 적용
result <- udpipe_annotate(ud_model, x = tokenized_df$word)

# 데이터 정리
lemma_data <- as.data.frame(result)
print(lemma_data[, c("token", "lemma")])

# lemma에서 첫 번째 어근만 추출
lemma_data <- lemma_data %>%
  mutate(lemma_clean = str_extract(lemma, "^[^+]+"))

# 결과 확인
print(lemma_data)

# 두 글자 이상 단어 필터링
filtered_data <- lemma_data %>%
  filter(str_length(lemma_clean) >= 2)
# 결과 확인
print(filtered_data)


# 2글자 이상 5글자 이하 단어만 추출하고 컬럼명 변경 후 word만 남기기
filtered_data <- lemma_data %>%
  filter(str_length(lemma_clean) >= 2 & str_length(lemma_clean) <= 5) %>%  # 2~5글자 필터링
  rename(word = lemma_clean) %>%                                           # 컬럼명 변경
  select(word)  

# 결과 확인
print(filtered_data)

write.csv(filtered_data, "data/youtube_cleaned_text_어근.csv", row.names = FALSE, fileEncoding = "UTF-8")
